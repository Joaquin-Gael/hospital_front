...
import { Service, ServiceCreate, ServiceUpdate } from '../../services/interfaces/hospital.interfaces';
...
// Interfaz para el formulario, incluye campos adicionales
...
export class ServiceListComponent implements OnInit {
  ...
  formFields: FormField[] = [
    ...
    { key: 'price', label: 'Precio', type: 'number', required: true, validators: [Validators.min(0)] },
    { key: 'specialty_id', label: 'ID de Especialidad', type: 'text', required: true },
  ...
  onDelete(service: ExtendedService): void {
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      data: { title: 'Eliminar Servicio', message: `¿Está seguro de eliminar el servicio "${service.name}"?` },
    });
  ...
  onView(service: ExtendedService): void {
    alert(`Detalles del servicio: ${service.name}\nDescripción: ${service.description}\nPrecio: $${service.price.toFixed(2)}\nDuración: ${service.duration || 'N/A'} min\nActivo: ${service.isActive ? 'Sí' : 'No'}`);
  }
  ...
  onFormSubmit(formData: ServiceFormData): void {
    ...
    if (this.formMode === 'create') {
      this.serviceService.createService(serviceData as ServiceCreate).subscribe({
        next: (newService) => {
          this.services.push({
            ...newService,
            duration,
          ...
          this.showForm = false;
          this.logger.info(`Servicio "${newService.name}" creado correctamente`);
        },
        error: (error: HttpErrorResponse) => {
      ...
      this.serviceService.updateService(this.selectedService.id.toString(), serviceData as ServiceUpdate).subscribe({
        next: (updatedService) => {
          const index = this.services.findIndex(s => s.id === updatedService.id);
          if (index !== -1) {
            this.services[index] = {
              ...updatedService,
              duration: duration !== undefined ? duration : this.services[index].duration,
              isActive: isActive !== undefined ? isActive : this.services[index].isActive,
            };
          }
          this.formLoading = false;
          this.showForm = false;
          this.logger.info(`Servicio "${updatedService.name}" actualizado correctamente`);
        },
        error: (error: HttpErrorResponse) => {
